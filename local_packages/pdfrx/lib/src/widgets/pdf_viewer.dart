...
class _PdfViewerState extends State<PdfViewer> with SingleTickerProviderStateMixin {
  ...
  final _pageLowResImages = <int, _PdfImageWithScale>{};
  final _pageLowResTimers = <int, Timer>{};
  final _pageFadeStarts = <int, DateTime>{};
  final _pageFadeTimers = <int, Timer>{};
  ...
  void _releaseAllImages() {
    ...
    for (final timer in _pageLowResTimers.values) {
      timer.cancel();
    }
    _pageLowResTimers.clear();
    ...
    _pageImages.clear();
    for (final image in _pageLowResImages.values) {
      image.image.dispose();
    }
    _pageLowResImages.clear();
    for (final t in _pageFadeTimers.values) {
      t.cancel();
    }
    _pageFadeTimers.clear();
    _pageFadeStarts.clear();
  ...
  void _cancelFadeForPage(int pageNumber) {
    _pageFadeTimers.remove(pageNumber)?.cancel();
    _pageFadeStarts.remove(pageNumber);
  }
  ...
  Future<void> _loadDelayed() async {
    ...
    final startPageNumber = _pageNumber ?? widget.initialPageNumber;
  ...
  bool _programmaticNavigationInFlight = false;
  ...
  void _setCurrentPageNumber(int? pageNumber, {bool doSetState = false}) {
    try {
      // ignore: avoid_print
      print('pdfrx._setCurrentPageNumber called: pageNumber=$pageNumber previous=$_pageNumber programmatic=$_programmaticNavigationInFlight');
    } catch (_) {}
    // If a programmatic navigation is in flight, avoid letting intermediate
    // layout-driven updates overwrite the requested target. Only allow the
    // final target to update the current page after the animation completes.
    if (!_programmaticNavigationInFlight) {
      _gotoTargetPageNumber = pageNumber;
    }

    if (_programmaticNavigationInFlight) {
      if (pageNumber == null) return;
      if (_gotoTargetPageNumber != null && pageNumber != _gotoTargetPageNumber) {
        // ignore intermediate pageNumber while animation in flight
        return;
      }
    }
    ...
    if (pageNumber != null && _pageNumber != pageNumber) {
      ...
      // Trigger windowed progressive loading around the new current page
      Future.microtask(() async {
        try {
          await _document?.loadPagesProgressively(startPageNumber: pageNumber, pageWindowSize: 5);
        } catch (_) {
          // ignore errors from background loading
        }
      });
    }
  }
  ...
  void _customPaint(ui.Canvas canvas, ui.Size size) {
    ...
    for (int i = 0; i < _document!.pages.length; i++) {
      ...
      final lowRes = _pageLowResImages[page.pageNumber];
      if (realSize != null) {
        // If a fade is in progress, crossfade low-res -> high-res
        final fadeStart = _pageFadeStarts[page.pageNumber];
        if (fadeStart != null && lowRes != null) {
          final elapsed = DateTime.now().difference(fadeStart).inMilliseconds;
          const fadeMs = 200;
          final t = (elapsed / fadeMs).clamp(0.0, 1.0);

          // draw low-res underneath with decreasing opacity
          final lowPaint = Paint()
            ..filterQuality = FilterQuality.low
            ..colorFilter = ui.ColorFilter.mode(Colors.white.withValues(alpha: 1.0 - t), BlendMode.modulate);
          canvas.drawImageRect(
            lowRes.image,
            Rect.fromLTWH(0, 0, lowRes.image.width.toDouble(), lowRes.image.height.toDouble()),
            rect,
            lowPaint,
          );

          // draw high-res above with increasing opacity
          final hiPaint = Paint()
            ..filterQuality = FilterQuality.high
            ..colorFilter = ui.ColorFilter.mode(Colors.white.withValues(alpha: t), BlendMode.modulate);
          canvas.drawImageRect(
            realSize.image,
          ...
          // no fade: draw high-res normally
          canvas.drawImageRect(
            realSize.image,
            Rect.fromLTWH(0, 0, realSize.image.width.toDouble(), realSize.image.height.toDouble()),
            rect,
        ...
        if (lowRes != null) {
          // Draw low-res image scaled up while high-res is loading.
          canvas.drawImageRect(
            lowRes.image,
            Rect.fromLTWH(0, 0, lowRes.image.width.toDouble(), lowRes.image.height.toDouble()),
            rect,
          ...
          // Draw a low-res placeholder background until low-res render completes.
          canvas.drawRect(
            rect,
            Paint()
      ...
      if (realSize == null) {
        _requestLowResPlaceholder(page, scale);
      }
  ...
  Future<void> _cachePageImage(PdfPage page, double width, double height, double scale) async {
    ...
    await synchronized(() async {
      ...
      // start crossfade from low-res if present
      if (_pageLowResImages.containsKey(page.pageNumber)) {
        _pageFadeStarts[page.pageNumber] = DateTime.now();
        _pageFadeTimers[page.pageNumber]?.cancel();
        _pageFadeTimers[page.pageNumber] = Timer.periodic(const Duration(milliseconds: 16), (t) {
          // request repaint while fading
          _invalidate();
          final elapsed = DateTime.now().difference(_pageFadeStarts[page.pageNumber]!).inMilliseconds;
          if (elapsed >= 220) {
            t.cancel();
            _pageFadeTimers.remove(page.pageNumber);
            _pageFadeStarts.remove(page.pageNumber);
            _invalidate();
          }
        });
      }
  ...
  Future<void> _requestLowResPlaceholder(PdfPage page, double deviceScale) async {
    if (_pageLowResImages.containsKey(page.pageNumber)) return;
    _pageLowResTimers[page.pageNumber]?.cancel();
    if (!mounted) return;
    // Schedule immediately but debounce small flurries
    _pageLowResTimers[page.pageNumber] = Timer(Duration(milliseconds: 8), () => _cacheLowResImage(page, deviceScale));
  }

  Future<void> _cacheLowResImage(PdfPage page, double deviceScale) async {
    if (!mounted) return;
    if (_pageLowResImages[page.pageNumber] != null) return;
    // Target small short side to make a fast thumbnail (in device pixels)
    const targetShortSide = 300; // px
    final pageShortSide = min(page.width, page.height);
    final smallScale = (targetShortSide / pageShortSide).clamp(0.15, 1.0);
    final width = (page.width * smallScale).toDouble();
    final height = (page.height * smallScale).toDouble();
    ...
    await synchronized(() async {
      ...
      final newImage = _PdfImageWithScale(await img.createImage(), smallScale);
      if (!mounted || cancellationToken.isCanceled) {
        img.dispose();
        newImage.dispose();
        return;
      }
      _pageLowResImages[page.pageNumber]?.dispose();
      _pageLowResImages[page.pageNumber] = newImage;
      img.dispose();
      _invalidate();
    });
  }
  ...
  void _removeCacheImagesForPage(int pageNumber) {
    ...
    final removedLow = _pageLowResImages.remove(pageNumber);
    if (removedLow != null) {
      removedLow.image.dispose();
    }
    _cancelFadeForPage(pageNumber);
  }
  ...
  void _onWheelDelta(Offset delta) {
    _startInteraction();
    final m = _txController.value.clone();
    final dx = -delta.dx * widget.params.scrollByMouseWheel! / _currentZoom;
    final dy = -delta.dy * widget.params.scrollByMouseWheel! / _currentZoom;
  ...
  Future<void> _goTo(Matrix4? destination, {Duration duration = const Duration(milliseconds: 200), Curve curve = Curves.easeInOut}) async {
    void update() {
  ...
  Future<void> _goToPage({
    ...
    try {
      // ignore: avoid_print
      print('pdfrx._goToPage called: requested=$pageNumber anchor=$anchor duration=${duration.inMilliseconds} curve=$curve');
    } catch (_) {}
    ...
    // Mark that a programmatic navigation is in flight so intermediate
    // layout-driven page guesses don't overwrite the final page number.
    _programmaticNavigationInFlight = true;
    try {
      try {
        // ignore: avoid_print
        print('pdfrx._goToPage navigating: target=$targetPageNumber pageCount=$pageCount (programmatic start)');
      } catch (_) {}

      await _goTo(_calcMatrixForPage(pageNumber: targetPageNumber, anchor: anchor), duration: duration, curve: curve);
      // Ensure final page number is set after the animation completes.
      _setCurrentPageNumber(targetPageNumber, doSetState: true);
    } finally {
      _programmaticNavigationInFlight = false;
      try {
        // ignore: avoid_print
        print('pdfrx._goToPage completed: target=$targetPageNumber (programmatic end)');
      } catch (_) {}
    }
  }
  ...
  Future<void> _goToRectInsidePage({
    ...
    _gotoTargetPageNumber = pageNumber;
    await _goTo(_calcMatrixForRectInsidePage(pageNumber: pageNumber, rect: rect, anchor: anchor), duration: duration, curve: curve);
...
class PdfViewerController extends ValueListenable<Matrix4> {
  ...
  Future<void> goToPage({
    ...
    Duration duration = const Duration(milliseconds: 200),
    Curve curve = Curves.easeInOut,
  }) => _state._goToPage(pageNumber: pageNumber, anchor: anchor, duration: duration, curve: curve);
  ...
  Future<void> goToRectInsidePage({
    ...
    Duration duration = const Duration(milliseconds: 200),
    Curve curve = Curves.easeInOut,
  }) => _state._goToRectInsidePage(pageNumber: pageNumber, rect: rect, anchor: anchor, duration: duration, curve: curve);
