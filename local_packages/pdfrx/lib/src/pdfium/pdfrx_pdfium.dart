...
class _PdfDocumentPdfium extends PdfDocument {
  ...
  Future<void> loadPagesProgressively<T>({
    ...
    if (startPageNumber != null) {
      ...
      final totalPageCount = _pages.length;
      final pageNumbers = _pageNumbersAround(startPageNumber, totalPageCount, pageWindowSize)
          .where((pageNumber) => !_pages[pageNumber - 1].isLoaded)
          .toList(growable: false);
      if (pageNumbers.isEmpty) return;

      final result = await _loadPagesAroundInLimitedTime(
        pageNumbers: pageNumbers,
        timeout: loadUnitDuration,
      );
      if (isDisposed) return;
      _pages = List.unmodifiable(result.pages);
      subject.add(PdfDocumentPageStatusChangedEvent(this, result.pagesLoaded));

      if (onPageLoadProgress != null) {
        for (final page in result.pagesLoaded) {
          if (!await onPageLoadProgress(page.pageNumber, totalPageCount, data)) {
            return;
          }
        }
      }
  ...
  List<int> _pageNumbersAround(int pageNumber, int totalPageCount, int windowSize) {
    final pageNumbers = <int>[];
    final start = pageNumber.clamp(1, totalPageCount);
    pageNumbers.add(start);

    for (int offset = 1; pageNumbers.length < windowSize; offset++) {
      final prev = start - offset;
      final next = start + offset;
      if (prev >= 1) {
        pageNumbers.add(prev);
        if (pageNumbers.length >= windowSize) break;
      }
      if (next <= totalPageCount) {
        pageNumbers.add(next);
        if (pageNumbers.length >= windowSize) break;
      }
      if (prev < 1 && next > totalPageCount) {
        break;
      }
    }
    return pageNumbers;
  ...
  Future<({List<_PdfPagePdfium> pages, List<_PdfPagePdfium> pagesLoaded})> _loadPagesAroundInLimitedTime({
    ...
    final results = await (await backgroundWorker).compute(
      (params) {
        ...
        return using((arena) {
          final pages = <({int pageNumber, double width, double height, int rotation})>[];
          final t = params.timeoutUs != null ? DateTime.now().add(Duration(microseconds: params.timeoutUs!)) : null;
          for (final pageNumber in params.pageNumbers) {
          ...
          return (pages: pages, totalPageCount: pdfium.FPDF_GetPageCount(doc));
        });
      },
      (
    ...
    for (final pageData in results.pages) {
      ...
      pages[pageNumber - 1] = loadedPage;
      loadedPages.add(loadedPage);
    }
    return (pages: pages, pagesLoaded: loadedPages);
  }
